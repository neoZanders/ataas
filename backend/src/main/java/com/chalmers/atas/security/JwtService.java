    ...
    public Result<Claims> parseRefreshClaims(String token) {
        try {
            ...
            if (!"refresh".equals(tokenType)) {
                return Result.errorFromCode(ErrorCode.INVALID_REFRESH_TOKEN, "Wrong token type");
            }

            return Result.ok(claims);
        } catch (ExpiredJwtException e) {
            return Result.errorFromCode(ErrorCode.REFRESH_TOKEN_EXPIRED);
        } catch (JwtException | IllegalArgumentException e) {
            return Result.errorFromCode(ErrorCode.INVALID_REFRESH_TOKEN);
        }
    }
