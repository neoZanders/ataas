...
public class ChocoAlgorithmTest extends ChocoAlgorithmTestBase {
    ...
    private void printBestPenalty(
            Result<AlgorithmResult> naiveResult,
            Result<AlgorithmResult> randomResult,
            Result<AlgorithmResult> customResult
    ) {
        Map<Integer, String> modelByPenalty = new HashMap<>();

        modelByPenalty.put(
                naiveResult.getData().totalPenalty(),
                "Naive " + modelByPenalty.getOrDefault(naiveResult.getData().totalPenalty(), "")
        );

        modelByPenalty.put(
                randomResult.getData().totalPenalty(),
                "Random " + modelByPenalty.getOrDefault(randomResult.getData().totalPenalty(), "")
        );

        modelByPenalty.put(
                customResult.getData().totalPenalty(),
                "Custom " + modelByPenalty.getOrDefault(customResult.getData().totalPenalty(), "")
        );
